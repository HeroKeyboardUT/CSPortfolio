...
vm_rg_struct *init_vm_rg(int rg_start, int rg_endi);
int enlist_vm_rg_node(vm_rg_struct **rglist, vm_rg_struct *rgnode);
int enlist_pgn_node(pgn_t **pgnlist, int pgn);
int vmap_page_range(pcb_t *caller, int addr, int pgnum, 
                    framephy_struct *frames, vm_rg_struct *ret_rg);
int vm_map_ram(pcb_t *caller, int astart, int send, int mapstart, int incpgnum, vm_rg_struct *ret_rg);
int alloc_pages_range(pcb_t *caller, int incpgnum, framephy_struct **frm_lst);
...
/* VM prototypes */
int pgalloc(pcb_t *proc, uint32_t size, uint32_t reg_index);
int pgfree_data(pcb_t *proc, uint32_t reg_index);
int pgread(
...
int find_victim_page(mm_struct *mm, int *pgn);
vm_area_struct *get_vma_by_num(mm_struct *mm, int vmaid);

/* MEM/PHY prototypes */
int MEMPHY_get_freefp(memphy_struct *mp, int *fpn);
int MEMPHY_put_freefp(memphy_struct *mp, int fpn);
int MEMPHY_read(memphy_struct *mp, int addr, BYTE *value);
int MEMPHY_write(memphy_struct *mp, int addr, BYTE data);
int MEMPHY_dump(memphy_struct *mp);
int init_memphy(memphy_struct *mp, int max_size, int randomflg);

/* Print list */
int print_list_fp(framephy_struct *fp);
int print_list_rg(vm_rg_struct *rg);
int print_list_vma(vm_area_struct *rg);
int print_list_pgn(pgn_t *ip);
int print_pgtbl(pcb_t *ip, uint32_t start, uint32_t end);
